...
def trading_listing(engine:Union[str, List[str], np.ndarray]='stock', market:Union[str, List[str], np.ndarray]='shares',
    ...
    for idx in tqdm(np.arange(args['engine'].shape[0]), desc='Fetching data', disable=not verbose):
        ...
        while True:
            ...
            dfs.append(pd.read_csv(fr"https://iss.moex.com/iss/history/engines/{args['engine'][idx]}/markets/{args['market'][idx]}/listing.csv?start={args['idx_st'][idx]+iteration*100}&status={args['status'][idx]}&lang={args['lang'][idx]}", encoding='ANSI', header=1, sep=';', on_bad_lines='skip'))
            iteration += 1
            if dfs[-1].shape[0] == 0:
                break
            #else:
